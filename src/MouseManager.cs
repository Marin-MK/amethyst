...
public class MouseManager
{
    ...
    public void ProcessMouseMoving(MouseEventArgs Args, bool Root)
    {
        ...
        Inside = Accessible && Widget.Viewport.Rect.Contains(Args.X, Args.Y);
        if (Accessible && OldInside != Inside) Widget.OnHoverChanged?.Invoke(Args);
        if (Root) ResetEventStates();
    }
    ...
    public void ProcessMouseDown(MouseEventArgs Args, bool Root)
    {
        ...
        CurrentArgs = Args;
        if (LeftMouseTriggered) LeftStartedInside = Inside;
        if (RightMouseTriggered) RightStartedInside = Inside;
        if (MiddleMouseTriggered) MiddleStartedInside = Inside;
        if (Accessible)
        {

...
public class TabView : Widget
{
    ...
    public TabView(IContainer Parent) : base(Parent)
    {
        ...
        Sprites["titles"] = new Sprite(Viewport);
        Sprites["filler"] = new Sprite(Viewport);
        Sprites["filler"].Y = 21;
    ...
    public void SetSelectedIndex(int SelectedIndex)
    {
        if (this.SelectedIndex != SelectedIndex)
        {
            ...
            OnTabChanged?.Invoke(new BaseEventArgs());
            Redraw();
        }
    }
    ...
    protected override void Draw()
    {
        ...
        {
            Size s = Font.TextSize(Tabs[i].Title);
            width += s.Width + 10;
            HeaderSizes.Add(s.Width + 10);
        }
        Sprites["titles"].Bitmap = new Bitmap(width, 20);
        Sprites["titles"].Bitmap.Unlock();
        Sprites["titles"].Bitmap.Font = Font;
        int textx = SelectedIndex == 0 ? 0 : 2;
        for (int i = 0; i < Tabs.Count; i++)
        {
            bool Selected = i == SelectedIndex;
            int w = Font.TextSize(Tabs[i].Title).Width + 10;
            Sprites["titles"].Bitmap.DrawText(Tabs[i].Title, textx + w / 2, Selected ? 2 : 4, TextColor, DrawOptions.CenterAlign);
            textx += w;
        }
    ...
    public TabContainer CreateTab(string Title)
    {
        ...
        tc.SetPosition(1, 21);
        tc.SetTitle(Title);
        tc.SetSize(Size.Width - 2, Size.Height - 22);
    ...
    public override void SizeChanged(BaseEventArgs e)
    {
        ...
        for (int i = 0; i < Tabs.Count; i++)
        {
            Tabs[i].SetSize(Size.Width - 2, Size.Height - 22);
        }
    }
